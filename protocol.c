...
int protocol_init_send(int sock, int video, int audio)
{
    ...
    if (tcp_send(sock, data, sizeof(data)) > 0)
        return 0;
    return -1;
}
...
int protocol_init_recv(int sock, int *video, int *audio)
{
    ...
    res = tcp_recv(sock, data, sizeof(data));
    if (res != sizeof(data))
    {
        if (res)
...
int protocol_resp_send(int sock, char const *error)
{
    ...
    return tcp_send(sock, (uint8_t const *)error, size);
}

int protocol_resp_recv(int sock, char *error, int max_size)
{
    int res = tcp_recv(sock, (uint8_t *)error, max_size - 1);
    if (-1 == res)
    {
...
{
    uint8_t btype = type;
    return tcp_send2(sock, &btype, sizeof(btype), data, size);
}

int protocol_packet_recv(int sock, struct Message *message)
{
    return message_recv(message, sock);
}
